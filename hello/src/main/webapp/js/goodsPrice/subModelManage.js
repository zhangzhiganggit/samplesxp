//由此开始联动
        ...
        function fillSubModelSelect(modNameLabel,subModSelected,subModNameLabel,defaultMsg){
        	var modId = $("#"+modNameLabel).val();
        	$.ajax({
                url: '/subModel/getSubModByMod',
                data : {
                	modId : modId
    	        },
                async:false,
                cache: true,
                success : function(subModelList) {
               	 	if(subModelList != null && subModelList !=undefined){
               	 		var options = "<option value=''>"+defaultMsg+"</option>";
        	            for(var i=0;i<subModelList.length;i++){
        	            	if(subModSelected != ''){
        	            		if(subModelList[i].subModId == subModSelected){
        	            			 options+="<option selected='selected' value=\""+subModelList[i].subModId+"\">"+subModelList[i].subModNm+"</option>";
        	            		 }else{
        	            			 options+="<option value=\""+subModelList[i].subModId+"\">"+subModelList[i].subModNm+"</option>";
        	            		 }
        	            	 }else{
    	            			 options+="<option value=\""+subModelList[i].subModId+"\">"+subModelList[i].subModNm+"</option>";
        	            	 }
        	             }
        	             $("#"+subModNameLabel).html(options);
        	             $("#"+subModNameLabel).trigger("chosen:updated");
