...
function(a) {
	...
	b.initializers = [], b.count = 0, b.VALID_NODES = ["P", "PRE", "BLOCKQUOTE", "H1", "H2", "H3", "H4", "H5", "H6", "DIV", "LI", "TD", "TH"], b.LANGS = [], b.DEFAULTS = {
		...
		this.$element.find("img[data-fr-image-pasted]").each(a.proxy(function(b, c) {
			this.options.pasteImage ? (0 === c.src.indexOf("data:") ? this.options.pastedImagesUploadURL && (setTimeout(a.proxy(function() {
				this.showImageLoader(), this.$progress_bar.find("span").css("width", "100%").text("请稍等!"), this.showByCoordinates(a(c).offset().left + a(c).width() / 2, a(c).offset().top + a(c).height() + 10), this.isDisabled = !0
			}, this), 10), a.ajax({
	...
	a.Editable.image_commands = {
		...
		this.addListener("hidePopups", this.hideImageEditorPopup), this.options.imageTitle && a('<div class="f-popup-line f-image-alt">').append('<label><span data-text="true">标题</span>: </label>').append(a('<input type="text">').on("mouseup keydown", function(a) {
			var b = a.which;
			b && 27 === b || a.stopPropagation()
		...
		void 0 === a && (a = !1), a ? this.$progress_bar.find("span").css("width", "100%").text("请稍等!") : this.$image_wrapper.find("h4").addClass("uploading"), this.$image_wrapper.find("#f-image-list-" + this._id).hide(), this.$progress_bar.show(), this.showInsertImage()
		...
		var d = '<img class="fr-fin" data-fr-image-preview="' + c + '" alt="图片标题" src="' + b + '" width="' + this.options.defaultImageWidth + '">',
		...
		if (void 0 !== b && b.length > 0) {
			...
			if (c) {
				...
				this.options.crossDomain ? e = this.createCORSRequest("POST", this.options.imageUploadURL) : (e = new XMLHttpRequest, e.open("POST", this.options.imageUploadURL)), e.onload = a.proxy(function() {
					this.$progress_bar.find("span").css("width", "100%").text("请稍等!");
					try {
	...
	a.Editable.prototype.showLinkWrapper = function() {
		...
		a += '<h4><span data-text="true">插入链接</span><a target="_blank" title="Open Link" class="f-external-link" href="#"><i class="fa fa-external-link"></i></a><i title="Cancel" class="fa fa-times" id="f-link-close-' + this._id + '"></i></h4>', a += '<div class="f-popup-line fr-hidden"><input type="text" placeholder="Text" class="f-lt" id="f-lt-' + this._id + '"></div>';
		var b = "";
		...
		return a += '<div class="f-popup-line"><button data-text="true" type="button" class="f-ok" id="f-ok-' + this._id + '">OK</button>', this.options.unlinkButton && (a += '<button type="button" data-text="true" class="f-ok f-unlink" id="f-unlink-' + this._id + '">UNLINK</button>'), a += "</div></div>"
