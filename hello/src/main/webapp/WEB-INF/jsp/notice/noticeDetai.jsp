...
<body class="no-skin ">
							...
							<div class="widget-box widget-color-normal2" style="opacity: 1;">
								<div class="widget-header">
									<h5 class="widget-title bigger lighter">
										<i class="ace-icon fa fa-table"></i> 公告详情
									</h5>
									...
									<div class="widget-main no-padding ">
										<div class="list-div" id="listDiv">
											<form id="formB" name="formB" method="post"
												...
												<input name="entry" value="board" hidden="hidden" /> <input
													name="entry1" value="update" hidden="hidden" /> <input
													name="noticeId" value="${notice.noticeId}" hidden="hidden" />
												<input name="noticeContent" hidden="hidden"
													id="noticeContent" /><input name="id" hidden="hidden"
													id="id" />
												<table style="width: 120%">
															...
															<div class="form-group form-group-sm width-300px">
																...
																<div class="col-sm-8  no-padding">
																	<select name="noticeType" id="noticeType"
																		...
																		<option value="业务公告"
																			<c:if test="${notice.noticeType=='业务公告' }">selected="selected"</c:if>>业务公告</option>
																		<option value="系统更新公告"
																			<c:if test="${notice.noticeType=='系统更新公告' }">selected="selected"</c:if>>系统更新公告</option>
																		<option value="增值产品公告"
																			<c:if test="${notice.noticeType=='增值产品公告' }">selected="selected"</c:if>>增值产品公告</option>
																	</select>
																</div>
															</div>
														</td>
															...
															<div class="form-group form-group-sm width-300px">
																...
																<div class="col-sm-8  no-padding">
																	<select name="noticeRange" id="noticeRange"
																		class="chosen-select form-control width-200px">
																		<option value="全部"
																			<c:if test="${notice.noticeRange=='全部' }">selected="selected"</c:if>>全部</option>
																		<option value="一代机构管理者"
																			<c:if test="${notice.noticeRange =='一代机构管理者'}">selected="selected"</c:if>>一代机构管理者</option>
																		<option value="机构管理者"
																			<c:if test="${notice.noticeRange =='机构管理者'}">selected="selected"</c:if>>机构管理者</option>
																		<option value="内部员工"
																			<c:if test="${notice.noticeRange =='内部员工'}">selected="selected"</c:if>>内部员工</option>
																		<option value="业务员"
																			<c:if test="${notice.noticeRange =='业务员'}">selected="selected"</c:if>>业务员</option>
																		<option value="实名认证业务员"
																			<c:if test="${notice.noticeRange =='实名认证业务员'}">selected="selected"</c:if>>实名认证业务员</option>
															...
															<div class="form-group form-group-sm width-300px">
																...
																<div class="col-sm-8  no-padding">
																	<select id="areaId" name="areaIds" multiple=”multiple”
																		...
																		<c:if test="${flg=='0' }">
																			<option value="1"
																				<c:if test="${fn:contains(notice.areaNm, '中国')}">selected="selected"</c:if>>全国</option>
																			<c:forEach items="${areaList }" var="prov">
																				<option value="${prov.areaId }"
																					<c:if test="${fn:contains(notice.areaNm, prov.areaNm )}">selected="selected"</c:if>>${prov.areaNm}</option>
																			</c:forEach>
																		</c:if>
																		<c:if test="${flg=='1' }">
																			<c:forEach items="${areaList }" var="prov">
																	...
																	<div class="col-sm-8  no-padding">
																		<div class="input-daterange input-group width-200px"
																			...
																			<input type="text" name="beginDt"
																				value="<c:if test="${notice.beginDate==null || notice.beginDate==''}">${nowDate }</c:if><c:if test="${notice.beginDate!=null && notice.beginDate!=''}"><fmt:formatDate value="${notice.beginDate}" pattern="yyyy-MM-dd"/></c:if>"
																				id="beginDt"
													...
													<tr>
														...
														<td>
															<div class="form-group form-group-sm width-100">
															<input type="hidden" id="noticeStatusVal" value ="${notice.noticeStatus}" />
																<label for="form-field-6"
																	class="col-sm-4 control-label widget-color-normal5 width-100px no-padding-left"><span
																	style="color: red">*</span>是否发布:</label>
																<div class="col-sm-8  no-padding">
																	<input type="checkbox" id="noticeStatus"
																		name="noticeStatus" />发布
																</div>
															</div>
														</td>
													</tr>
													...
													<tr>
														<td colspan=2>
															<div class="form-group form-group-sm width-100">
																<h4 class="header smaller lighter black">
																	阅读次数：${notice.readingTimes }&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
																	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;发布人：${notice.inputEmpId }&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;发布日期：
																	<fmt:formatDate value="${notice.beginDate}"
																		pattern="yyyy-MM-dd" />
																</h4>
															</div>
														</td>
	...
	window.onload = function(){
		var noticeStatus = document.getElementById("noticeStatusVal").value;
		if(noticeStatus.trim()=='已发布'){
			$("#noticeStatus").prop("checked",true)
			id.val("1")
		}else{
			$("#noticeStatus").prop("checked",false)
			id.val("0")
		}
	}
	
		$("#noticeStatus").change(function(){
			if($("#noticeStatus").prop("checked")){
				id.val("1")
			}else{
				id.val("0")
			}
    ...
    function closeWin(){
     ...
     function noticeAdd(){
		...
		function noticeDetail() {
			...
			if (noticeCruces == "" || noticeCruces == null
					|| getLen(noticeCruces) > 160) {
				alert("要点总结为1-160个字符，请检查");
				return false;
			}
			if (pattern.test(noticeCruces)) {
