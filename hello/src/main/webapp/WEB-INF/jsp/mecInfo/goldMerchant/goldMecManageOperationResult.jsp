							...
							<div class="widget-box widget-color-normal3">
								<div class="widget-header">
									<h5 class="widget-title bigger lighter">
										...
										<%-- <c:if test="${null eq col }">
											<c:if test="${entry=='all'}">
											本次操作成功${success}条，操作失败${error}条
											</c:if>
										</c:if> --%>
										<%-- <c:if test="${null != col }">
											${col }
										...
										<c:if test="${success != null && error != null && tip == null}">
											本次操作成功${success}条，操作失败${error}条。
										</c:if>
										<c:if test="${success != null && error != null && tip == true}">
											本次操作成功${success}条，${error }。
										</c:if>
										<c:if test="${success != null && error == null }">
											${success }
										</c:if>
										<c:if test="${col != null}">
											${col }
...
function toList(){
	window.parent.location.href="<%=request.getContextPath()%>/goldMerchant/goldMerchantList"
		+"?mno=${listMno}"
