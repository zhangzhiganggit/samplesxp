									...
									<div class="widget-main no-padding ">
										<div class="widget-main ">
											<form id="searchForm" class="form-inline form-horizontal"
												method="post"  action="<%=request.getContextPath()%>/manufacturer/addOneMf"
												>
												<div class=" col-xs-12">
													<div class="form-group form-group-sm width-100 ">
														...
														<div class="col-sm-8  no-padding width-70">
															<input type="text" name="factShortname" id="factShortname"
																 maxlength="20"
																 onblur="checkFactShortname()"
													...
													<div class="form-group form-group-sm width-300px">
													 ...
													 <div class="form-group form-group-sm width-100 ">
                                                        ...
                                                        <div class="col-sm-8 no-padding width-85">
                                                            <textarea  id="remark"  maxlength="1000" title="不能超过1000个字符" name="remark" style="height:80px" class="form-control limited input-small width-100"></textarea>
                                                        </div>
                                                    </div>
												</div>
		...
		if(factShortname!=null && factShortname!=''){
			 $.ajax({
	 				...
	 				        url:'<%=request.getContextPath() %>/manufacturer/checkName',
	 				        data:'factShortname='+factShortname+'&flg=1',
	 						async : false,
	 						error : function(data) {
	 							//data = eval("(" + data + ")")
	 							alert(data);
			...
			$('#searchForm').formValidation({
				...
				fields : {
					...
					factUrls : {
						validators : {
							 regexp: {
		                          regexp:/^(http|www|ftp|)?(:\/\/)?(\w+(-\w+)*)(\.(\w+(-\w+)*))*((:\d+)?)(\/(\w+(-\w+)*))*(\.?(\w)*)(\?)?(((\w*%)*(\w*\?)*(\w*:)*(\w*\+)*(\w*\.)*(\w*&)*(\w*-)*(\w*=)*(\w*%)*(\w*\?)*(\w*:)*(\w*\+)*(\w*\.)*(\w*&)*(\w*-)*(\w*=)*)*(\w*)*)$/g,
		                          message : "xxxxx"
							 }
						}
					},
