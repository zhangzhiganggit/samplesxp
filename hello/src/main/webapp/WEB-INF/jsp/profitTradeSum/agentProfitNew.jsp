...
<body class="no-skin ">
                                ...
                                <div class="widget-body">
                                    <div class="widget-main no-padding ">
                                        <div class="widget-main ">
                                            <form class="form-inline form-horizontal" action="<%=request.getContextPath()%>/orgTradeSumNew/orgProfitNew.do" id="searchForm" method="post">
                                            ...
                                            <form  action="<%=request.getContextPath()%>/orgTradeSumNew/downExcel" id="downLoadForm" method="post">
                                            </form>
                                    ...
                                    <div class="widget-main no-padding ">
                                        <table class="table table-striped table-bordered table-hover dataTable no-footer DTTT_selectable">
                                            ...
                                            <tr>
                                                ...
                                                <th style="text-align: center;width: 7%">POS收单分润</th>
                                                <th style="text-align: center;width: 7%">其他收单分润</th>
                                                <th style="text-align: center;width: 7%">产品分润</th>
                                                <th style="text-align: center;width: 6%">营销奖励</th>
                                                <th style="text-align: center;width: 6%">损失实扣</th>
                                                <th style="text-align: center;width: 7%">最终发放金额</th>
													 ...
													 <c:if test="${de.profitSts eq '1' }"> 
                                                       ...
                                                       <td class="uuid" style="display: none;">${de.uuid}</td>
														...
														<td style="text-align: right;">
														     <a href="#" onclick="seeOtherAmount('${de.orgNo }','${year}','${month}','2')" >
										                          ${de.posProfitAmountSumDesc}
										                     </a>
														</td>
														<td style="text-align: right;">
														     <a href="#" onclick="seeOtherAmount('${de.orgNo }','${year}','${month}','3')" >
										                          ${de.otherProfitAmountSumDesc }
										                     </a>
														</td>
														<td style="text-align: right;"> 
															<a href="#" onclick="seeOtherAmount('${de.orgNo }','${year}','${month}','4')" >
																${de.productAmountSumDesc }
															</a>
														</td>
														<td style="text-align: right;">
														     <a href="#" onclick="seeOtherAmount('${de.orgNo }','${year}','${month}','5')" >
                                                                  ${de.marketAmountSumDesc}
										                     </a>
														</td>
														<td style="text-align: right;">
														     <a href="#" onclick="seeOtherAmount('${de.orgNo }','${year}','${month}','9')" >
                                                                  ${de.buckleAmountSumDesc}
										                     </a>
														</td>
														
														<td style="text-align: right;">
														 ${de.issueAmountSumDesc }
														</td>
														<td style="text-align: right;">
    ...
    function typeDetailShow(destDt,orgNo){
  	  $.fancybox.open({
				href : '<%=request.getContextPath() %>/profitLoss/lossDetail?destDt='+destDt+'&orgNo='+orgNo,
				type: 'iframe',
	        padding: 5,
	        scrolling: 'no',
	        fitToView: true,
    ...
    function seeOtherAmount(orgNo,year,month,hisType){
    	$.fancybox.open({
    		href : '<%=request.getContextPath()%>/profitImportNew/showOneOrgView.do?orgNo='+orgNo+'&year='+year+'&month='+month+'&hisType='+hisType,
			type: 'iframe',
	        padding: 5,
	        scrolling: 'no',
	        fitToView: true,
    ...
    function seeAllOtherAmount(orgNo,year,month){
    	$.fancybox.open({
    		href : '<%=request.getContextPath()%>/profitImportNew/showOneOrgDetailView.do?orgNo='+orgNo+'&year='+year+'&month='+month,
			type: 'iframe',
	        padding: 5,
	        scrolling: 'no',
	        fitToView: true,
    ...
    function importView(){
    	$.fancybox.open({
    		href : '<%=request.getContextPath()%>/profitImportNew/showImportView.do',
			type: 'iframe',
	        padding: 5,
	        scrolling: 'no',
	        fitToView: true,
	        ...
	        closeClick: false,
	        afterClose:function(){
	        	window.location.href='<%=request.getContextPath()%>/orgTradeSumNew/orgProfitNew.do';
			}
        });
    };
    ...
    function uploadExcelAgentProfit(){
		...
		$("#searchForm").attr("action","<%=request.getContextPath()%>/profitExportNew/AgentProfitExcelDown.do");
       ...
       function batchEdit(){
    	  ...
    	  if(res){
        	  ...
        	  $.ajax({
    			   type: "POST",
    			   url: "<%=request.getContextPath() %>/orgTradeSumNew/openToAgent.do",
    		       data : {jsonStr:jsonStr,flg:2},
    			   success: function(data){
    				   alert(data.msg);
    				   window.location.href='<%=request.getContextPath() %>/orgTradeSumNew/orgProfitNew.do?orgNo='+$("#orgNo").val()+'&profitMonth='+$("#profitMonth").val()+'&orgNm='+$("#orgNm").val()+'&branchOrgNm='+$("#branchOrgNm").val();
    			   }
    			});
    	  }
      ...
      function openAllOperForm( operService ){
          $.fancybox.open({
              href : '<%=request.getContextPath()%>/orgTradeSumNew/' + operService,
              type: 'iframe',
              padding: 5,
              scrolling: 'no',
              fitToView: true,
