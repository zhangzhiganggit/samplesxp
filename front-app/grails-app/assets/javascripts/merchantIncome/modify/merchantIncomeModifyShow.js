...
if (typeof jQuery !== 'undefined') {
    $(function(){
         ...
         * */
        var ifBegin=false;
        var registNameOld_yes_array = [],registNameOld_no_array = [];
            ...
            if("registName"==$(this).attr("id")){
                ...
                $.ajax({
                    ...
                    success: function (json) {
                        ...
                        if (json.code == "1") {
                            console.log("注册名称Yes")
                            registNameOld_yes_array.push(findVal);//存入成功数组

                            ctrlAccountLegalName();// ****>显示隐藏'账户法人姓名'属性
                        }else {
                            registNameOld_no_array.push(findVal);//存入失败数组
         ...
         * */
        var registCodeOld_yes_array = [],registCodeOld_no_array = [];
        var settleManIdNumber_yes_array = [],settleManIdNumber_no_array = [];
        var toPublicAccountName_yes_array = [],toPublicAccountName_no_array = [];
        var settleAccountNo_yes_array = [],settleAccountNo_no_array = [];
        $('#registCode,#settleManidNumber,#toPublicAccountName,#toPrivateSettleAccountNo,#toPublicSettleAccountNo').blur(function(){
            ...
            if("registCode"==$(this).attr("id")){
                ...
                // ****>重复次数校验
                //若数组中存在值，则说明刚才已经被校验了，直接清空并提示，不再进行校验
                if ($.inArray(findVal, registCodeOld_yes_array) >= 0) {
                    //是已经校验过直接通过的值，不进行服务器校验
                    return false;
                }
                if ($.inArray(findVal, registCodeOld_no_array) >= 0) {
                    //已经校验过的，非法值，不进行服务器校验
                ...
                $.ajax({
                    ...
                    success: function (json) {
                        if (json.code == "1") {
                            ...
                            registCodeOld_yes_array.push(findVal);//存入成功数组
                        }else {
                            registCodeOld_no_array.push(findVal);//存入失败数组
                            OKDialogCallBack("注册登记号的重复次数超出系统限制，请联系相关人员！",'registCode');
                        }
                    },
                    error: function (xhr, status) {
            ...
            if("settleManidNumber"==$(this).attr("id")){
                ...
                // ****>重复次数校验
                if ($.inArray(findVal, settleManIdNumber_yes_array) >= 0) {
                    return false;
                }
                if ($.inArray(findVal, settleManIdNumber_no_array) >= 0) {
                ...
                $.ajax({
                    ...
                    success: function (json) {
                        if (json.code == "1") {
                            ...
                            settleManIdNumber_yes_array.push(findVal);//存入成功数组
                        }else {
                            settleManIdNumber_no_array.push(findVal);//存入失败数组
                            OKDialogCallBack("结算人身份证号的重复次数超出系统限制，请联系相关人员！",'settleManidNumber');
                        }
                    },
                    error: function (xhr, status) {
            ...
            if("toPublicAccountName"==$(this).attr("id")){
                ...
                // ****>重复次数校验
                if ($.inArray(findVal, toPublicAccountName_yes_array) >= 0) {
                    return false;
                }
                if ($.inArray(findVal, toPublicAccountName_no_array) >= 0) {
                    OKDialogCallBack("对公结算卡账户名的重复次数超出系统限制，请联系相关人员！",'toPublicAccountName');
                    return false;
                }
                $.ajax({
                    ...
                    success: function (json) {
                        if (json.code == "1") {
                            ...
                            toPublicAccountName_yes_array.push(findVal);//存入成功数组

                            // ****>显示隐藏'账户法人姓名'属性
                            ctrlAccountLegalName();
                        }else {
                            toPublicAccountName_no_array.push(findVal);//存入失败数组
                            OKDialogCallBack("对公结算卡账户名的重复次数超出系统限制，请联系相关人员！",'toPublicAccountName');
                        }
                    },
                    error: function (xhr, status) {
            ...
            if("toPrivateSettleAccountNo"==$(this).attr("id") || "toPublicSettleAccountNo"==$(this).attr("id")){
                ...
                // ****>重复次数校验
                //若数组中存在值，则说明刚才已经被校验了，直接清空并提示，不再进行校验
                if ($.inArray(findVal, settleAccountNo_yes_array) >= 0) {
                    return false;
                }
                if ($.inArray(findVal, settleAccountNo_no_array) >= 0) {
                ...
                $.ajax({
                    ...
                    success: function (json) {
                        if (json.code == "1") {
                            console.log("结算账号Yes")
                            settleAccountNo_yes_array.push(findVal);//存入成功数组
                        }else {
                            settleAccountNo_no_array.push(findVal);//存入成功数组
                            OKDialogCallBack(showlbl+"结算账号的重复次数超出系统限制，请联系相关人员！",showId);
                        }
                    },
                    error: function (xhr, status) {
    ...
    function findMerchantInfoFun(thisButton,merchantCode){
        ...
        $.ajax({
            ...
            success:function(json){
                ...
                $('#linkmanName').val(data.tBapUsrIfCrpPo.contNmCn);//回填-联系人姓名
                $('#linkmanMobileNo').val(data.tBapUsrIfCrpPo.contTelNo);// 回填-联系人手机号电话
                ...
                if(data.tBapMecIfDetailPo.cprRegAddr != null && data.tBapMecIfDetailPo.cprRegAddr != '' && data.tBapMecIfDetailPo.cprRegAddr.indexOf('-') != -1){
                    var array = data.tBapMecIfDetailPo.cprRegAddr.split('-');
                    //回填注册地址到select2控件
...
console.log("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~开始回填结算信息~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
                if(flag == '1'){
                    ...
                    if(data.newTZeroStmt == 1){
                        ...
                        $("#ifIntradaySettle").val("1");//todo 设置值啥意思？
                        ...
                        $('#ifIntradaySettle').prop('checked',false);
                        $("#ifIntradaySettle").val("0");//todo 设置值啥意思？
                    }
...
console.log(data.tBapUsrStmIfPo);
                    ...
                    if(data.tBapUsrStmIfPo != undefined && data.tBapUsrStmIfPo.stmTm != undefined){
                        ...
                        if(tpsLen > 0){
                            ...
                            $("#merchantDayChangeTime").val(timePoint);//日切时间 TODO 为啥取第一个值
...
console.log("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~开始赋值结算卡信息~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
                        ...
                        if( stmBanks[defaultSettleCardIndex].lbnkNo != 'undefined' && stmBanks[defaultSettleCardIndex].lbnkNo != '' ){
                            $.ajax({
                                ...
                                success : function(resultJson) {
                                    $("#toPrivateOpenbankInfomation").append(new Option(resultJson.items[0].text,resultJson.items[0].linkBankNo, false, true));
                                }
                            });
                        }
                    ...
                    //     //如果存在两张结算卡，需要获取非默认结算卡的下标  显示为取消卡2
                        if(stmBanks.length >= 2){
                            ...
                            $('#settleManidNumber2').val(stmBanks[secondSettleCardIndex].uuid); //结算信息2uuid
                            if(stmBanks[secondSettleCardIndex].lbnkNo != 'undefined' && stmBanks[secondSettleCardIndex].lbnkNo != ''){
                                $.ajax({
                                    ...
                                    cache: true,
                                    success : function(resultJson) {
                                        $("#toPublicOpenbankInfomation").append(new Option(resultJson.items[0].text,resultJson.items[0].linkBankNo, false, true));
                                    }
                                });
                            }
                            // for(var i=0;i<hoSettleCardInputIds.length;i++){
                ...
                if(mainteFee=='01'){
                    ...
                    //$('#mtfee').val(data.tBapMecFeeInfoPo.mtFee);//收取金额 TODO 找不到对应的值
        ...
        $.ajax({
            ...
            success: function (posModelList) {
                if (posModelList != null && posModelList != undefined) {
                    ...
                    //TODO 测试环境缺少默认pos型号S90-G临时添加使用
                    if(posModelValue == 'S90-G'){
                        options += "<option selected='selected' value=\"" + 'S90-G' + "\">" + 'S90-G' + "</option>";
                    }
...
function changeStmtType(val,f,flag){
    ...
    if(flag=='second'){
        ...
        //             $("#zftFlg").trigger('chosen:updated');
        //         }
        //     }else{
        //         if($("#zftFlg").prop('checked')){
        //             var res = confirm("确认将结算类型修改为对公，将取消朝付通业务?");
        //             if (res) {
    ...
    $.ajax({
        ...
        success: function(data) {
            if(data == 1){
                ...
                if(settleAccountType2 != undefined){
