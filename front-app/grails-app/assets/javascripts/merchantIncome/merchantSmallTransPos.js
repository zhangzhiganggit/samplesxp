...
if (typeof jQuery !== 'undefined') {
    $(function(){
         ...
         * */
        $('#registCode,#registName,#settleManidNumber,#toPublicAccountName,#toPrivateSettleAccountNo,#toPublicSettleAccountNo').blur(function () {
           ...
           /*     var mecNormalLevel=$('#mecNormalLevel').val();
            ...
            //获取商户等级类型
            //var mecNormalLevel = judgeLevelOne($('#normalLevel').val());
            /**<注册登记号> blur 事件绑定*/
                    ...
                    $.ajax({
                        ...
                        success: function (json) {
                            if (json.code == "1") {
                                ...
                                //不清数据
                                OKDialogCallBack("结算人身份证号的重复次数超出系统限制，请联系相关人员！", '');
            ...
            if ("toPrivateSettleAccountNo" == $(this).attr("id") || "toPublicSettleAccountNo" == $(this).attr("id")) {
                ...
                if(ifCheckRepeat('toPublicSettleAccountNo',$(this).val(),mecNormalLevel)) {
                    $.ajax({
...
//结算卡二身份证号校验
 ...
 /*   var rateInfo = $("#rateInfomationID").val();
    ...
    $('#commitBtn').click(function(){
        var mecNormalLevel = $("#mecNormalLevel").val();
        // if(mecNormalLevel==null||mecNormalLevel==''){
        //     bootboxAlert("请选择商户等级！")
        //     $("#commitBtn").attr("disabled",true)
        //     return false
        // }
        if(!checkImage(mecNormalLevel)){
            return false;
        }
        ...
        if(ifEmpty($('#settleManidNumber').val())){
            OKDialog("历史记录结算人身份证为空,无法提交");
            return false;
        }
        ...
        if(!sett){
            OKDialog("对公结算信息必须整组填写！");
            return false;
        }

     /*   if($("#toPublicAccountName").val().trim().length>0){
        ...
        if (findVal != oldMap['settleManidNumber']) {
            $.ajax({
                url: '/merchantIncome/settleCardNoRepeat',
                data: {merchantType: repeatType, cardNo: findVal,mecNormalLevel:mecNormalLevel},
                type: "POST",
        ...
        if ( findVal.length>0&&findVal != oldMap['toPublicAccountName'] ) {
            $.ajax({
                url: '/merchantIncome/settleAccountNameRepeat',
                data: {merchantType: repeatType, accoutName: findVal,mecNormalLevel:mecNormalLevel},
                type: "POST",
        ...
        if ( findVal.length>0&&findVal != oldMap['toPublicSettleAccountNo'] ) {
            $.ajax({
                url: '/merchantIncome/settleAccountNoRepeat',
                data: {merchantType: repeatType, accoutNo: findVal,mecNormalLevel:mecNormalLevel},
                type: "POST",
        ...
        if ( findVal.length>0&&findVal != oldMap['toPrivateSettleAccountNo'] ) {
            $.ajax({
                url: '/merchantIncome/settleAccountNoRepeat',
                data: {merchantType: repeatType, accoutNo: findVal,mecNormalLevel:mecNormalLevel},
                type: "POST",
                ...
                success: function (json) {
                    if (json.code == "1") {
                        ...
                        //不清空
                        OKDialogCallBack("个体结算账号的重复次数超出系统限制，请联系相关人员！", '');
                        flag = false;
                    }
                },
                error: function (xhr, status) {
        ...
        if(chkWHT){
            ...
            if(!chkPOS){
                ...
                if($('#ifMaintainFeeOfPos2').prop("checked")!=true){
                    ...
                    getShowMessage($('#ifMaintainFeeOfPos2'),"3","red",1,"Alt");
                ...
                if($('#ifMaintainFeeOfPos3').prop("checked")!=true){
                    ...
                    getShowMessage($('#ifMaintainFeeOfPos3'),"3","red",1,"Alt");
    ...
    function verifySettleInfo(){
        var pubName=$('#toPublicAccountName').val().trim();
        var pubNo=$('#toPublicSettleAccountNo').val().trim();
        var pubCode=$('#toPublicCnapsCode').val().trim();
        var pubInfo=$('#toPublicOpenbankInfomation').val().trim();
            // 2. 判断【对公】结算是否整组填写
    ...
    function startPCA(provincespan,cityspan,areaspan,name_province_val,name_city_val,name_area_val,provinceHidden,cityHidden,areaHidden){
        ...
        $("#"+provincespan+"").on('change',function () {
           ...
           // getShowMessage($("#"+name_province_val+""),"2","red",1,"Alt");
        ...
        $("#"+cityspan+"").on('change',function () {
            ...
            getShowMessage($("#"+name_city_val+""),"2","red",1,"Alt");
        });
        ...
        $("#"+areaspan+"").on('change',function () {
           ...
           getShowMessage($("#"+name_area_val+""),"2","red",1,"Alt");
        });
...
function clearSettleCard2Info(){
    ...
    for(var i=0;i<attr.length;i++){
        ...
        //触发修改提示记录
        //$("#"+attr[i]+"").trigger("blur");
        getShowMessage($("#"+attr[i]+""),"2","red",1,"Alt");
    }
    getShowMessage($("#settleAccountType2"),"2","red",1,"Alt");
}
...
function mecNormalLevelValidate_Small(){
 ...
 * 2、同时在费率那块的页面展示中增加提示：该商户为打包秒到商户，不允许预设费率（这个提示由计费系统給，具体以计费返回参数为准）
 */
function getCurrentRateInfo(inMno,agreementNo,mercPreferenceType) {
    $.ajax({
        url: '/merchantIncomeModify/getMecAgreementByMerc',
        data:{
            inMno:inMno,
